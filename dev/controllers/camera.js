...
exports.orderCameras = (req, res, next) => {
  ...
  let queries = [];
  // Récupère à présent un tableau de produits
  ////for (let productId of req.body.products) {
  for (let product of req.body.products) {
    const queryPromise = new Promise((resolve, reject) => {
      // Il faut donc récupérer la valeur id différemment
      ////Camera.findById(product).then(
      Camera.findById(product.Id).then(
        (camera) => {
          if (!camera) {
            ////reject('Camera not found: ' + productId);
            reject('Camera not found: ' + product.Id);
          }
          camera.imageUrl = req.protocol + '://' + req.get('host') + '/images/' + camera.imageUrl;
          // On ajoute 2 données au retour, la quantité et la lentille choisie
          camera.quantity = product.Quantity;
          camera.lense = product.Lense;
          resolve(camera);
        }
      ).catch(
        () => {
